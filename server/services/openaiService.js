...
const SYSTEM_PROMPT = `You extract structured search parameters from a user's question about recent bird sightings.
...
- Use "" for location if none is given, including phrases that refer to the user's own position such as "near me", "nearby", "around me", "my location", "in my location", "my area", or "my current location". A missing location is fine — never set needsClarification because of it.
...
- Set needsClarification to true only if the question is not about finding recent bird sightings, and put a short friendly question in clarifyingQuestion. Otherwise clarifyingQuestion is "".`;
...
async function interpretQuestion(question) {
  ...
  return {
    birdName: typeof parsed.birdName === "string" ? parsed.birdName.trim() : "",
